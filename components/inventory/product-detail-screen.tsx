...
export function ProductDetailScreen({ id }: { id: string }) {
  ...
  return <div className="min-h-dvh bg-background pb-10">
    ...
    <main className="mx-auto max-w-3xl px-4 pb-24 pt-5 md:px-8 md:pt-8">
      ...
      <section className="mt-7"><h2 className="text-base font-medium">最近记录</h2><div className="mt-3 flex flex-col divide-y divide-border/50 rounded-[22px] bg-card px-4 ring-1 ring-border/60">{history.map((item) => <div key={item} className="flex items-center justify-between py-3 text-sm"><span className="text-muted-foreground">{item.split(' · ')[0]}</span><span className="text-foreground">{item.split(' · ')[1]}</span></div>)}<Link href={`/history?product=${encodeURIComponent(product.name)}`} className="flex items-center justify-between py-3 text-sm text-muted-foreground">查看全部记录<ChevronRight className="size-4" /></Link></div></section>
    </main>
